...
This can be achieved in the **Connections** section of the {{ short_name }} dialog:

![Connections panel](img/quickstart_servers.png)

## Adding connections

Click ![add](img/icons/add.svg){: width="16" } **Add...** and choose one of the supported server types to create a new server connection:
...
Fill in the required parameters and click the **Save** button to store the connection details.
...
If you wish to remove a server connection, select it from the left panel and click the ![remove](img/icons/remove.svg){: width="16" } **Remove** button.
The ![duplicate](img/icons/duplicate.svg){: width="16" } **Duplicate** button may come in handy if you wish to try out an alternative configuration based on an existing
server connection.

{{ short_name }} will automatically store all server connections in the QGIS settings for later use.
However, sometimes you might want to backup or share all the configured server connections. The ![export](img/icons/export.svg){: width="16" } **Export** button will
allow you to *save* a JSON file of the current configuration.
The ![import](img/icons/import.svg){: width="16" } **Import** button will do the opposite and *load* server connections from a JSON file. Note that the imported connections will be
...
![GeoNetwork connection](img/servers_geonetwork.png)
...
!!! note
    In order to convert the QGIS layer metadata into a format that GeoNetwork understands,
...
![GeoServer connection](img/servers_geoserver.png)
...
2. [Import into PostGIS database (direct connect)](#import-into-postgis-database-direct-connect) (vector layers only)
3. [Import into PostGIS database (managed by GeoServer)](#import-into-postgis-database-managed-by-geoserver) (vector layers only)
...
GeoServer will merely establish a link to the imported database table for the created layer(s). This will require a direct [PostGIS](#postgis) connection, which you can set up in the *Connections* section of {{ short_name }} as well (see below).
Note that GeoServer should also have read access to that same PostGIS connection.

!!! note
    This method is only available for vector layers.

    For raster layers, {{ short_name }} will resort to [file-based storage](#file-based-storage) instead.
...
1. This method requires the GeoServer [Importer extension](https://docs.geoserver.org/stable/en/user/extensions/importer/installing.html) to be installed and enabled.
...
3. Created PostGIS tables must be deleted manually. Please read [this warning](#datastore-selection) for more information.
...
![Datastore selection](img/servers_geoserver2.png)
...
Alternatively, you can click the ![add](img/icons/add.svg){: width="16" } button to create a new datastore yourself, if you know the connection details.
...
Setting up this connection is required when using the [direct connect](#import-into-postgis-database-direct-connect) option in a [GeoServer](#geoserver) connection.

![PostGIS connection](img/servers_postgis.png)
...
![MapServer connection](img/servers_mapserver.png)
...
![MapServer storage](img/servers_mapserver2.png)
