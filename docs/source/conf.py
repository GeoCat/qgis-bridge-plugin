...
doc_name = f'{app_name} QGIS Plugin Documentation'
copyright = f"{datetime.now().year}, {author}. Built from {commit_id}"
...
version = ".".join(str(p) for p in meta.semanticVersion(release))
...
latex_documents = [
  ('index', 'geocatbridge.tex', doc_name, author, 'manual')
]
...
.. |menu_name| replace:: `Web/{app_name}`
.. |long_name| replace:: {project}
...
# man_pages = [
#     ('index', app_name, doc_name, [author], 1),
#     ('2_background_and_requirements', app_name, doc_name, [author], 1),
#     ('3_installation', app_name, doc_name, [author], 1),
#     ('4_configuration', app_name, doc_name, [author], 1),
# ]
