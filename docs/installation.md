...
QGIS users who use a QGIS version *beyond* 3.16 should verify that the `lxml` [Python dependency](server_configuration.md#geonetwork) is available.
...
-   If you wish to let GeoServer import your data into PostGIS (i.e. through
    ...
    [Importer extension](https://docs.geoserver.org/latest/en/user/extensions/importer/index.html)
...
1. Open the **QGIS Plugin Manager** by clicking on the **Plugins → Manage and Install Plugins...** menu entry.

    ![Plugin Manager menu](img/pluginmanagermenu.png)

    You will now see a dialog similar to the one below, listing all your (pre-)installed plugins:

    ![Plugin Manager](img/pluginmanager.png)

2. Click the **All** section on the sidebar to list all available plugins. Scroll down to find "{{ app_name }}"
   ...
   will display detailed information about the plugin:
    ...
    ![Plugin Manager - Bridge](img/pluginmanagerbridge.png)

3. Click **Install Plugin** and wait until {{ short_name }} has been downloaded and installed.

4. Close the Plugin Manager when done. You should now have a new menu entry under the **Web** menu,
   which contains a couple of menu items related to the {{ short_name }} plugin:

    ![Bridge menu entry](img/bridgemenuentry.png)
...
Alternatively, you can update plugins manually using the **Upgrade All** button.
