...
!!! note
    You may experience that even if your layer is of one of the types listed above,
...
Please see [Server Connections](server_configuration.md) for more information on how to add these. Then come back to this page
to continue the publication process.

In the {{ short_name }} dialog, navigate to the **Publish** section.

![Publish section](img/publish_section.png)

Here you can choose the supported layers that you wish to publish, by (un)ticking the checkboxes for each layer:

![Publish layer list](img/publish_layers_list.png)

If you click on one of the layers in the *Layers panel*, you can see that the **Metadata** and **Attributes** tabs on the right side update
their contents based on the currently selected layer. This will allow you to:

-   Edit the layer metadata (see [Metadata Editing](metadata_editing.md))
...
![Publish fields](img/publish_fields.png)

Once you have finished configuring the properties for all map layers that you wish to publish,
please select the destinations server(s) that you would like to use in the **Online** tab at the bottom,
or specify the things you wish to export to a local file on the **Offline** tab.
...
Those servers must be selected from the two available drop-down lists in the **Online** tab.
As mentioned before, these servers should be added beforehand in the **Connections** section of the {{ short_name }} dialog,
as described in [Server Connections](server_configuration.md).

!!! note
    If you are publishing layers to GeoServer, the destination workspace name will be taken from the
...
{{ short_name }} dialog (e.g. bad credentials). Please go to the [Server Connections](server_configuration.md) section to fix the issue(s).

![Publish server error](img/publish_server_error.png)

Once you have selected the layers you wish to publish and the destination(s) have been set, click
on the **Publish** button to start the publication process. Note that the **Publish** button
is context-aware, meaning that when the **Online** tab is active, the data will be published online only.

Alternatively, if the **Offline** tab is active and the **Publish** button is pressed, the data will only be
published offline. Please see [Export files](#export-files) and [Removing geodata and metadata](#removing-geodata-and-metadata) as well.

!!! tip
    If you have a lot of data and you wish to continue working with QGIS while layer data is being published,
    you can tick the **Background** checkbox next to the **Publish** button.

    After you hit the **Publish** button, the {{ short_name }} dialog will close and you will be able to continue working with QGIS.
...
Once you have uploaded your geodata and wish to update the symbology of your server layers without uploading all layer data again, check the **Symbology only** option.
The styles of the selected (checked) layers will then be updated, but no layer data will be uploaded and/or overwritten.
For more information about how {{ short_name }} handles symbology, please read the [Supported Symbology](supported_symbology.md) section.
...
!!! warning
    **It is currently not possible to add layers to an existing workspace.**
    ...
    ![Workspace warning](img/publish_workspace_warning.png)
...
-   **View metadata**: If the metadata is already published to a catalog
    server, this option opens a browser to show the metadata from the
    server.

![Preview metadata](img/preview_gnmetadata.png)
...
![Preview layers](img/preview_layers.png)
...
If the **Online** tab is active and one or more servers have been selected for publication, a **Clear All** button becomes available.
...
!!! warning
    ...
    the database tables will not be removed when you hit the **Clear All** button.
...
To export your (meta)data and/or style files locally, use the **Offline** export tab in {{ short_name }}.
...
all the current selected (checked) layers when you click the **Publish** button.
