...
QMD_TO_ISO19139_XSLT = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "resources", "qgis-to-iso19139.xsl"
)
ISO19139_TO_QMD_XSLT = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "resources", "iso19139-to-qgis.xsl"
)
ISO19115_TO_ISO19139_XSLT = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "resources", "iso19115-to-iso19139.xsl"
)
WRAPPING_ISO19115_TO_ISO19139_XSLT = os.path.join(
    os.path.dirname(os.path.dirname(__file__)),
    "resources",
    "ISO19115-wrapping-MD_Metadata-to-ISO19139.xslt",
)
FGDC_TO_ISO19115 = os.path.join(
    os.path.dirname(os.path.dirname(__file__)),
    "resources",
    "ArcCatalogFgdc_to_ISO19115.xsl",
)
...
def loadMetadataFromIsoXml(layer, filename):
    ...
    QgsMessageLog.logMessage(
        "Exporting ISO19193 metadata to %s" % qmdFilename,
        "GeoCat Bridge",
        level=Qgis.Info,
    )
...
def loadMetadataFromEsriXml(layer, filename):
    ...
    QgsMessageLog.logMessage(
        "Exporting ISO19115 metadata to %s" % isoFilename,
        "GeoCat Bridge",
        level=Qgis.Info,
    )
    dom = ET.parse(filename)
...
def loadMetadataFromWrappingEsriXml(layer, filename):
    isoFilename = tempFilenameInTempFolder("fromesri.xml")
    QgsMessageLog.logMessage(
        "Exporting Wrapping-ISO19115 metadata to %s" % isoFilename,
        "GeoCat Bridge",
        level=Qgis.Info,
    )
    dom = ET.parse(filename)
...
def loadMetadataFromFgdcXml(layer, filename):
    ...
    QgsMessageLog.logMessage(
        "Exporting FGDC metadata to %s" % isoFilename, "GeoCat Bridge", level=Qgis.Info
    )
    dom = ET.parse(filename)
...
def saveMetadata(
    layer, mefFilename=None, apiUrl=None, wms=None, wfs=None, layerName=None
):
    ...
    apiUrl = apiUrl or ""
    transformedFilename = transformMetadata(
        filename, uuid, apiUrl, wms, wfs, layerName or layer.name()
    )
...
def createMef(uuid, metadataFilename, mefFilename, thumbnailFilename):
    ...
    z.write(
        metadataFilename,
        os.path.join(uuid, "metadata", os.path.basename(metadataFilename)),
    )
    z.write(
        thumbnailFilename,
        os.path.join(uuid, "public", os.path.basename(thumbnailFilename)),
    )
...
def getInfoXmlContent(uuid, thumbnailFilename):
    ...
    _addSubElement(
        public,
        "file",
        attrib={"name": os.path.basename(thumbnailFilename), "changeDate": d},
    )
