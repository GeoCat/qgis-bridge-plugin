...
def exportLayer(
    layer, fields=None, toShapefile=False, path=None, force=False, log=None
):
    ...
    if layer.type() == layer.VectorLayer:
        if toShapefile:
            if (
                force
                or layer.fields().count() != len(fields)
                or (os.path.splitext(filename.lower())[1] != ".shp")
            ):
                attrs = [
                    i
                ...
                if log is not None:
                    log.logInfo(
                        QCoreApplication.translate(
                            "GeocatBridge", "Layer %s exported to %s"
                        )
                        % (destFilename, output)
                    )
                return output
            ...
            force
            or os.path.splitext(filename.lower())[1] != ".gpkg"
            or layer.fields().count() != len(fields)
            or not isSingleTableGpkg(filename)
        ):
            attrs = [
                i
                for i, f in enumerate(layer.fields())
            ...
            QgsVectorFileWriter.writeAsVectorFormat(
                layer, output, "UTF-8", attributes=attrs
            )
            if log is not None:
                log.logInfo(
                    QCoreApplication.translate(
        ...
        if log is not None:
            log.logInfo(
                QCoreApplication.translate(
                    "GeocatBridge", "No need to export layer %s stored at %s"
                )
        ...
        if force or not filename.lower().endswith("tif"):
            ...
            writer.writeRaster(
                layer.pipe(), layer.width(), layer.height(), layer.extent(), layer.crs()
            )
            del writer
            if log is not None:
                log.logInfo(
