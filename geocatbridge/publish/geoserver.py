...
class GeoserverServer(ServerBase):
    ...
    def publishStyle(self, layer):
        ...
        self.logInfo(
            QCoreApplication.translate(
                "GeocatBridge", "Style for layer %s exported as zip file to %s"
            )
            ...
            if layer.dataProvider().name() == "postgres" and self.useOriginalDataSource:
                ...
                if layer.source() not in self._exportedLayers:
                    if self.storage == self.POSTGIS_MANAGED_BY_GEOSERVER:
                        path = exportLayer(
                            layer, fields, toShapefile=True, force=True, log=self
                        )
    ...
    def _publishVectorLayerFromPostgis(self, layer, db):
        ...
        ft = {"featureType": {"name": name, "srs": layer.crs().authid()}}
    ...
    def _publishVectorLayerFromFileToPostgis(self, layer, filename):
        ...
        if not isDataUploaded:
            _import = {
                "import": {
                    "targetStore": {"dataStore": {"name": datastoreName}},
                    "targetWorkspace": {"workspace": {"name": self._workspace}},
                }
            }
            ...
            target = {"dataStore": {"name": datastoreName}}
    ...
    def _setLayerStyle(self, layername, stylename):
        ...
        r = self.request(url)
        layer = r.json()
        styleUrl = "%s/workspaces/%s/styles/%s.json" % (
            self.url,
            self._workspace,
            stylename,
        )
        layer["layer"]["defaultStyle"] = {"name": stylename, "href": styleUrl}
        ...
        for ws in wss:
            ...
            if "dataStore" in root:
                for datastore in root["dataStore"]:
                    url = "%s/workspaces/%s/datastores/%s.json" % (
                        self.url,
                        ws,
                        datastore["name"],
                    )
    ...
    def validateGeodataBeforePublication(self, errors, toPublish):
        ...
        if not path:
            errors.add(
                "QGIS Project is not saved. Project must be saved before publishing layers to GeoServer"
            )
