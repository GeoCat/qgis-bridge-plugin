...
class MapserverServer(ServerBase):
    ...
    def publishLayer(self, layer, fields=None):
        ...
        exportLayer(
            layer, fields, toShapefile=True, path=layerPath, force=True, log=self
        )
    ...
    def closePublishing(self):
        ...
        mapElement = {
            ...
            "SCALEBAR": {"ALIGN": "CENTER", "OUTLINECOLOR": "0 0 0"},
        }
        mapElement["LAYERS"] = [
            {"INCLUDE": '"%s.txt"' % layer.name()} for layer in self._layers
        ]
        mapElement["SYMBOLS"] = [
            {"INCLUDE": '"%s_symbols.txt"' % layer.name()} for layer in self._layers
        ]
    ...
    def layerWmsUrl(self, name):
        ...
        return (
            "%s?map=%s/maps/%s.map&service=WMS&version=1.1.0&request=GetCapabilities&layers=%s"
    ...
    def layerWfsUrl(self):
        project = self.projectName()
        return (
            "%s?map=%s/maps/%s.map&service=WFS&version=2.0.0&request=GetCapabilities"
            % (self.url, project, project)
        )
