...
WIDGET, BASE = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "geocatwidget.ui")
)
...
class GeoCatWidget(WIDGET, BASE):
    def __init__(self, parent=None):
        ...
        self.txtAbout.linkClicked.connect(
            lambda url: webbrowser.open_new_tab(url.toString())
        )
    ...
    def login(self):
        ...
        try:
            ...
            self.labelUrlGeoserver.setText(
                "<a href='{0}'>{0}</a>".format(mygeocat.client.geoserverUrl)
            )
            self.labelUrlGeonetwork.setText(
                "<a href='{0}'>{0}</a>".format(mygeocat.client.geonetworkUrl)
            )
            self.labelStatusGeoserver.setText(
                self._statusText(mygeocat.client.geoserverStatus)
            )
            self.labelStatusGeoserver.setStyleSheet(
                self._statusCss(mygeocat.client.geoserverStatus)
            )
            self.labelStatusGeonetwork.setText(
                self._statusText(mygeocat.client.geonetworkStatus)
            )
            self.labelStatusGeonetwork.setStyleSheet(
                self._statusCss(mygeocat.client.geonetworkStatus)
            )
